...
function startOptimizedRoute() {
    ...
    activeTasks.forEach(req => {
        ...
        req.logisticsCondition = '4.5°C 冷鏈維持正常';
    });
...
function apiApproveRequest(reqId) {
    ...
    if (r) {
        r.status = '已核准出庫';
        r.dispatchTime = '準備派車';
        r.logisticsCondition = '溫控正常 (4.8°C)';
...
function renderInventoryTable() {
    ...
    thHtml += `<th>溫控要求</th><th>跨店調撥與預約決策</th></tr>`;
    ...
    dbInventory.filter(item => 
        item.drugChineseName.toLowerCase().includes(kw) || 
        ...
        let tempBadge = item.temperatureReq === 'Cold Chain 2-8°C' 
            ? `<span class="badge badge-info"><i class="fa-solid fa-snowflake"></i> 2-8°C冷鏈</span>` 
            : `<span class="badge badge-success">常溫</span>`;
    ...
    myOrders.forEach(req => {
        ...
        if (linkedTransfer) {
            ...
            if (linkedTransfer.status === '待審核') {
                ...
                stepperHtml = generateStepperMarkup(3, `物流車運行中 (溫控: ${linkedTransfer.logisticsCondition})`);
...
function renderInboxTable() {
    ...
    myOutbox.forEach(req => {
        ...
        else if (req.status === '專車配送中') badgeStr = `<span class="badge badge-warning"><i class="fa-solid fa-truck fa-spin"></i> 調配專車配送中 (溫控:${req.logisticsCondition})</span>`;
...
function renderDriverTasks() {
    ...
    driverTasks.forEach(req => {
        ...
        let tempTag = STATIONS_METADATA[req.from].hours.includes('冷鏈') || req.item.includes('胰島素')
            ? `<span class="badge badge-info" style="font-size:0.7rem; margin-top:3px;"><i class="fa-solid fa-snowflake"></i> 需開啟車載溫控 2-8°C</span>`
            : `<span class="badge badge-success" style="font-size:0.7rem; margin-top:3px;">常溫運輸</span>`;
        ...
        card.innerHTML = `
            ...
            <div style="margin-bottom:8px;">${tempTag}</div>
            ${actionHtml}
        `;
...
function apiDriverDepart(reqId) {
    ...
    if (r) {
        ...
        r.logisticsCondition = '4.5°C 冷鏈維持正常';
        localStorage.setItem('SmartPharma_Requests', JSON.stringify(dbRequests));
...
function apiDriverArrive(reqId) {
    ...
    if (r) {
        r.status = '已送達簽收';
        r.logisticsCondition = '4.8°C 送達簽章';
...
// =========================================================================
// 🧭 DRIVER LIVE GPS NAVIGATION & COLD CHAIN MONITOR SIMULATOR
// =========================================================================
...
function startLiveNavigationLoop(task) {
    ...
    if (liveSensorInterval) clearInterval(liveSensorInterval);
    ...
    liveSensorInterval = setInterval(updateColdChainSensors, 1000);
...
function updateLiveNavigation() {
    ...
    let directionText = "";
    if (navProgressPct === 0) {
        directionText = `【已出發】專車已從 ${startMeta.name} 啟程出發！裝載 ${activeNavTask.item} (x${activeNavTask.qty}盒)，開啟 2-8°C 冷鏈保溫箱全程監測。`;
        ...
        directionText = "【行駛中】通過百吉隧道。目前正在進入山區路段，海拔逐漸上升，冷鏈車廂溫度運作正常。";
        ...
        directionText = `【已抵達】已順利抵達終點 ${endMeta.name}！冷鏈箱全程控溫合格。請立刻辦理藥品點收簽發，完成交付。`;
    }
...
function updateColdChainSensors() {
    if (isTempAnomalyActive) {
        mockTemp += 0.35 + Math.random() * 0.15;
        if (mockTemp > 9.8) mockTemp = 9.8;
    } else {
        // Safe oscillation around 4.5°C
        mockTemp += (Math.random() - 0.5) * 0.12;
        if (mockTemp < 4.1) mockTemp = 4.1;
        if (mockTemp > 4.9) mockTemp = 4.9;
    }

    mockHumidity += Math.round((Math.random() - 0.5) * 2);
    if (mockHumidity < 44) mockHumidity = 44;
    if (mockHumidity > 52) mockHumidity = 52;

    const tempDisplay = document.getElementById('liveTempDisplay');
    const humDisplay = document.getElementById('liveHumidityDisplay');
    if (tempDisplay) tempDisplay.innerText = mockTemp.toFixed(1) + ' °C';
    if (humDisplay) humDisplay.innerText = mockHumidity + ' %';

    const tempGauge = document.getElementById('tempGaugeBox');
    const tempBadge = document.getElementById('tempStatusBadge');
    
    if (tempGauge && tempBadge && tempDisplay) {
        if (mockTemp > 8.0 || mockTemp < 2.0) {
            // Anomaly active
            tempGauge.style.background = '#ffe4e6';
            tempGauge.style.borderColor = '#f43f5e';
            tempDisplay.style.color = '#e11d48';
            tempBadge.innerHTML = `<span class="badge badge-danger" style="background:#fecaca; color:#e11d48; font-weight:800; animation: blinker 1s linear infinite;"><i class="fa-solid fa-circle-exclamation"></i> ⚠️ 溫度異常警報 (請檢查車廂空調!)</span>`;
            playAlertBeep();
        } else {
            // Normal
            tempGauge.style.background = '#f0fdf4';
            tempGauge.style.borderColor = '#bbf7d0';
            tempDisplay.style.color = '#16a34a';
            tempBadge.innerHTML = `<span class="badge badge-success" style="background:#d1fae5; color:#065f46; font-weight:800;"><i class="fa-solid fa-circle-check"></i> 溫控監測合格</span>`;
        }
    }

    // Dynamic IoT Telemetry Card updates
    const connStatus = document.getElementById('iotConnStatus');
    const sigStrength = document.getElementById('iotSignalStrength');
    const sigIcon = document.getElementById('iotSignalIcon');
    const iotProtocol = document.getElementById('iotProtocol');

    if (connStatus && sigStrength && sigIcon) {
        if (isTempAnomalyActive || mockTemp > 8.0 || mockTemp < 2.0) {
            connStatus.innerHTML = `
                <span class="live-dot" style="width:7px; height:7px; background:#ef4444; border-radius:50%; display:inline-block; animation: pulse-red 1s infinite;"></span>
                4G LTE (高頻警報傳輸中)
            `;
            connStatus.style.color = '#f87171';
            sigStrength.innerText = 'RSSI: -89dBm (重試傳輸中)';
            sigStrength.style.color = '#f59e0b';
            sigIcon.style.color = '#f59e0b';
            if (iotProtocol) {
                iotProtocol.innerHTML = 'MQTT v5.0 (Topic: <span style="color:#ef4444; font-weight:bold; animation: blinker 1s linear infinite;">fuxing/logistics/t03/alert</span>)';
            }
        } else {
            const randomRssi = -60 - Math.floor(Math.random() * 8);
            connStatus.innerHTML = `
                <span class="live-dot" style="width:7px; height:7px; background:#4ade80; border-radius:50%; display:inline-block; animation: pulse-green 1.5s infinite;"></span>
                4G LTE (已連線)
            `;
            connStatus.style.color = '#4ade80';
            sigStrength.innerText = `RSSI: ${randomRssi}dBm (極佳)`;
            sigStrength.style.color = '#e2e8f0';
            sigIcon.style.color = '#10b981';
            if (iotProtocol) {
                iotProtocol.innerHTML = 'MQTT v5.0 (Topic: fuxing/logistics/t03)';
            }
        }
    }
}
...
function simulateTempAnomaly() {
    isTempAnomalyActive = true;
    showToast("🚨 已啟動冷鏈故障模擬！車廂內溫度快速攀升中！", "error");
}

function resolveTempAnomaly() {
    isTempAnomalyActive = false;
    mockTemp = 4.5;
    showToast("✅ 冷鏈故障已排除，箱體溫度正恢復安全區間！", "success");
}
...
function playAlertBeep() {
    try {
        ...
        console.log("Audio alert skipped due to user interaction policy:", e);
    }
}
...
function renderAdminTransferTable() {
    ...
    transfers.forEach(req => {
        ...
        tr.innerHTML = `
            ...
            <td><strong>${req.item}</strong> (x${req.qty}盒)</td>
            <td>${progressStr}<br><small style="color:var(--text-muted);">${req.logisticsCondition||''}</small></td>
